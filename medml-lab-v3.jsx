...
async function trackEvent(event) {
  try {
    ...
    let count = 0;
    try { const r = await window.storage.get(key, true); count = parseInt(r?.value || "0"); } catch {}
    await window.storage.set(key, String(count + 1), true);
  } catch {}
}
async function getStats() {
  const events = ["tab_explorer","tab_methodology","tab_blueprint","methodology_generated","blueprint_generated","blueprint_saved"];
  const stats = {};
  for (const e of events) {
    try { const r = await window.storage.get(`analytics_${e}`, true); stats[e] = parseInt(r?.value||"0"); } catch { stats[e]=0; }
  }
  return stats;
}
...
const EXAMPLE_BLUEPRINTS = [
  {
    title: "VocalPD: Early Parkinson's Detection via Voice Biomarkers",
    topic: "AI for early Parkinson's detection using voice",
    purpose: "science-fair",
    author: "Priya S., Grade 11",
    award: "🥇 ISEF Finalist",
    question: "Can a machine learning model trained on vocal biomarkers (jitter, shimmer, HNR) distinguish early-stage Parkinson's disease from healthy controls with >90% sensitivity?",
    hypothesis: "A Random Forest classifier trained on 22 vocal features from the UCI Parkinson's dataset will achieve >90% sensitivity for detecting Parkinson's disease.",
    model: { name: "Random Forest + SVM Ensemble", reason: "Handles small dataset (195 samples) well; vocal features are tabular and non-linear" },
    datasets: [{ name: "UCI Parkinson's Dataset", url: "https://www.kaggle.com/datasets/vikasukani/parkinsons-disease-data-set", size: "195 recordings" }],
    timeline: [
      { week: "Weeks 1-2", task: "Literature review + UCI dataset download and EDA" },
      { week: "Weeks 3-4", task: "Feature engineering — compute jitter, shimmer, HNR from raw audio" },
      { week: "Weeks 5-6", task: "Train RF and SVM models, hyperparameter tuning, cross-validation" },
      { week: "Weeks 7-8", task: "Error analysis, write paper, build demo app" }
    ],
    metrics: ["Sensitivity ≥ 90% (primary — missing Parkinson's is the critical error)", "AUC-ROC ≥ 0.95", "Specificity ≥ 85%"],
    novelty: "Most existing models use only a subset of vocal features; this project systematically compares all 22 MDVP features and builds an explainable ensemble showing which features matter most."
  },
  {
    title: "SepsisGuard: 6-Hour Early Warning System Using ICU Vitals",
    topic: "Predicting sepsis risk from ICU time-series vitals",
    purpose: "hackathon",
    author: "Marcus T., Grade 12",
    award: "🏆 HackMIT Health Track Winner",
    question: "Can an LSTM model trained on 6-hour ICU vital sign windows predict sepsis onset with clinically actionable lead time?",
    hypothesis: "An LSTM trained on rolling 6-hour windows of heart rate, blood pressure, temperature, and SpO2 from MIMIC-III will achieve AUROC >0.83 for sepsis prediction 4+ hours before clinical recognition.",
    model: { name: "LSTM (2 layers, 64 hidden units)", reason: "Captures temporal dependencies in irregular ICU time-series; handles variable-length sequences naturally" },
    datasets: [{ name: "PhysioNet Challenge 2019", url: "https://physionet.org/content/challenge-2019/1.0.0/", size: "40,336 ICU patients" }],
    timeline: [
      { week: "Weeks 1-2", task: "PhysioNet data access, preprocessing pipeline, handle missing values via forward-fill" },
      { week: "Weeks 3-4", task: "LSTM architecture, training loop in PyTorch, early stopping" },
      { week: "Weeks 5-6", task: "Hyperparameter sweep, threshold optimization for clinical utility" },
      { week: "Weeks 7-8", task: "Demo dashboard, 48hr hackathon sprint, deployment" }
    ],
    metrics: ["AUROC > 0.83 on held-out test set", "False positive rate < 25% (alert fatigue threshold)", "Mean lead time > 4 hours before clinical sepsis recognition"],
    novelty: "Combines PhysioNet's utility score optimization with a clinician-facing alert dashboard showing which vital signs contributed most to the prediction."
  },
  {
    title: "DermAI: Democratizing Skin Cancer Screening in Underserved Communities",
    topic: "AI skin cancer detection for mobile screening",
    purpose: "research-paper",
    author: "Aisha K., Grade 11",
    award: "📄 BMC Medical Informatics Accepted",
    question: "Can a MobileNetV2 model fine-tuned on HAM10000 achieve dermatologist-equivalent performance for melanoma detection on smartphone-captured dermoscopy images?",
    hypothesis: "Transfer learning from ImageNet to HAM10000 using MobileNetV2 will achieve AUC ≥ 0.90 for melanoma classification while remaining deployable on edge devices.",
    model: { name: "MobileNetV2 (transfer learning)", reason: "89% smaller than EfficientNet-B4 while retaining 94% performance — critical for mobile deployment" },
    datasets: [{ name: "HAM10000", url: "https://www.kaggle.com/datasets/kmader/skin-lesion-analysis-toward-melanoma-detection", size: "10,015 images, 7 classes" }],
    timeline: [
      { week: "Weeks 1-2", task: "Dataset analysis, class imbalance quantification, augmentation strategy" },
      { week: "Weeks 3-4", task: "MobileNetV2 fine-tuning, focal loss for class imbalance" },
      { week: "Weeks 5-6", task: "Grad-CAM visualizations, demographic bias analysis across skin tones" },
      { week: "Weeks 7-8", task: "Paper writing, submission to BMC Medical Informatics" }
    ],
    metrics: ["Melanoma AUC ≥ 0.90 (primary)", "Balanced accuracy ≥ 82% across all 7 classes", "Inference time < 200ms on mobile CPU"],
    novelty: "First study to systematically evaluate HAM10000-trained models on skin tone subgroups using ITA score bucketing, revealing performance disparities in darker skin tones."
  }
];
...
/* SHARE */
.share-row { display:flex; gap:8px; flex-wrap:wrap; margin-top:16px; padding-top:14px; border-top:1px solid var(--border); }
.share-btn { display:inline-flex; align-items:center; gap:6px; font-family:'JetBrains Mono',monospace; font-size:0.68rem; padding:8px 14px; border-radius:8px; cursor:pointer; border:1px solid var(--border); background:var(--s2); color:var(--muted); transition:all .2s; text-decoration:none; letter-spacing:.4px; }
...
/* EXAMPLE BLUEPRINT CARDS */
.ex-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(280px,1fr)); gap:12px; margin-bottom:20px; }
.ex-card { background:var(--s2); border:1px solid var(--border); border-radius:12px; padding:18px; cursor:pointer; transition:all .2s; }
.ex-card:hover { border-color:var(--border2); transform:translateY(-2px); }
.ex-card.active { border-color:var(--green); background:rgba(0,255,157,.04); }
.ex-award { font-family:'JetBrains Mono',monospace; font-size:0.62rem; color:var(--yellow); margin-bottom:6px; }
.ex-title { font-family:'Barlow Condensed',sans-serif; font-size:1rem; font-weight:700; margin-bottom:4px; }
.ex-author { font-size:0.75rem; color:var(--muted); }
...
export default function App() {
  ...
  function goTab(i) {
    setTab(i);
    const names = ["tab_explorer","tab_methodology","tab_blueprint","tab_saved"];
    trackEvent(names[i]);
  }
...
<style>{CSS}</style>
      <div className="app">
        <nav className="nav">
          ...
          <div className="tabs">{TABS.map((t, i) => <button key={t} className={`tab ${tab===i?"active":""}`} onClick={() => goTab(i)}>{t}</button>)}</div>
        </nav>
        <div style={{marginBottom:28}}>
          ...
          <div className="tabs" style={{display:"inline-flex"}}>{TABS.map((t,i)=><button key={t} className={`tab ${tab===i?"active":""}`} onClick={()=>goTab(i)}>{t}</button>)}</div>
        </div>
        {tab===0 && <Explorer />}
        {tab===1 && <Methodology />}
        {tab===2 && <Blueprint />}
...
{"model":{"name":"string","architecture":"string","reason":"string"},"dataset":{"name":"string","url":"string","size":"string","note":"string"},"dataset2":{"name":"string","url":"string","size":"string"},"metrics":[{"name":"string","target":"string","why":"string"},{"name":"string","target":"string","why":"string"},{"name":"string","target":"string","why":"string"}],"challenges":["string","string","string"],"clinicalStake":"string","tips":["string","string"]}`);
      ...
      setResult(parsed);
      trackEvent("methodology_generated");
...
function Blueprint() {
  ...
  async function generate() {
    ...
    setLoading(true); setResult(null); setErr(""); setSaved(false); setActiveExample(null);
    try {
...
{"title":"string","question":"string","hypothesis":"string","impact":"string","datasets":[{"name":"string","url":"string","size":"string"},{"name":"string","url":"string","size":"string"}],"model":{"name":"string","reason":"string"},"timeline":[{"week":"Weeks 1-2","task":"string"},{"week":"Weeks 3-4","task":"string"},{"week":"Weeks 5-6","task":"string"},{"week":"Weeks 7-8","task":"string"}],"metrics":["string","string","string"],"novelty":"string"}`);
  ...
  function shareTwitter() {
    const text = `Just generated a healthcare AI research blueprint for "${result?.title || topic}" using MedML Lab 🧬 — free tool for students learning medical ML`;
    window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent("https://medmllab.vercel.app")}`, "_blank");
  }
  function shareLinkedIn() {
    window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent("https://medmllab.vercel.app")}`, "_blank");
  }
  async function copyLink() {
    await navigator.clipboard.writeText("https://medmllab.vercel.app");
    setCopied(true); setTimeout(() => setCopied(false), 2000);
  }

  const displayResult = activeExample || result;

  return (
    <div>
      {/* Example blueprints */}
      <div className="card">
        <div className="card-head">✨ Example Blueprints</div>
        <div className="card-sub">See what a finished blueprint looks like — click any to explore, or generate your own below.</div>
        <div className="ex-grid">
          {EXAMPLE_BLUEPRINTS.map((ex, i) => (
            <div key={i} className={`ex-card ${activeExample===ex?"active":""}`} onClick={() => { setActiveExample(activeExample===ex?null:ex); setResult(null); }}>
              <div className="ex-award">{ex.award}</div>
              <div className="ex-title">{ex.title}</div>
              <div className="ex-author">{ex.author}</div>
              <div style={{fontSize:".75rem",color:"var(--muted)",marginTop:6,lineHeight:1.5}}>{ex.question.slice(0,100)}...</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Generator */}
      <div className="card">
        <div className="card-head">📐 Generate Your Blueprint</div>
        ...
        <div className="out-section">
          <div className="card" style={{borderColor:"rgba(0,255,157,.3)",background:"rgba(0,255,157,.03)"}}>
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
              <div>
                {activeExample && <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:".65rem",color:"var(--yellow)",marginBottom:6}}>{activeExample.award} · {activeExample.author}</div>}
                <div className="out-label">Project Title</div>
                <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:"1.5rem",fontWeight:800,marginBottom:6}}>{displayResult.title}</div>
              </div>
              {!activeExample && <button className="btn btn-ghost" onClick={save} disabled={saved}>{saved?"✓ Saved":"💾 Save"}</button>}
            </div>
            <div style={{marginTop:14,display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              <div className="info-panel"><div className="info-panel-label">Research Question</div><p>{displayResult.question}</p></div>
              <div className="info-panel"><div className="info-panel-label">Hypothesis</div><p>{displayResult.hypothesis}</p></div>
              <div className="info-panel full" style={{gridColumn:"1/-1"}}><div className="info-panel-label">Real-World Impact</div><p>{displayResult.impact}</p></div>
            </div>
          </div>
          <div className="out-grid">
            <div className="out-card accent">
              <div className="out-label">Suggested Model</div>
              <div className="out-title">{displayResult.model?.name}</div>
              <div className="out-body" style={{marginTop:6}}>{displayResult.model?.reason}</div>
            </div>
            <div className="out-card">
              <div className="out-label">Datasets</div>
              {displayResult.datasets?.map(ds=>(
                <div key={ds.name} style={{marginBottom:10}}>
          ...
          <div style={{background:"var(--s2)",border:"1px solid var(--border)",borderRadius:10,padding:20,marginTop:12}}>
            ...
            <div className="timeline">
              {displayResult.timeline?.map((t,i)=>(
          ...
          <div className="out-grid" style={{marginTop:12}}>
            <div className="out-card">
              ...
              {displayResult.metrics?.map((m,i)=><div key={i} style={{display:"flex",gap:8,padding:"8px 0",borderBottom:"1px solid var(--border)"}}><div style={{color:"var(--green)"}}>→</div><div style={{fontSize:".83rem"}}>{m}</div></div>)}
            </div>
            <div className="out-card accent">
              <div className="out-label">What Makes This Novel</div>
              <div className="out-body" style={{fontSize:".85rem",lineHeight:1.7}}>{displayResult.novelty}</div>
            </div>
          </div>

          {/* Share buttons */}
          <div className="share-row">
            <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:".65rem",color:"var(--muted)",alignSelf:"center"}}>SHARE:</span>
            <div className="share-btn twitter" onClick={shareTwitter}>🐦 Twitter</div>
            <div className="share-btn linkedin" onClick={shareLinkedIn}>💼 LinkedIn</div>
            <div className="share-btn copy" onClick={copyLink}>{copied?"✓ Copied!":"🔗 Copy Link"}</div>
          </div>
        </div>
      )}
    </div>
